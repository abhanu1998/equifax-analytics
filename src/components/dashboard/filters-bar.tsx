...
export function FiltersBar() {
  const { state, applyPreset, manualRefresh } = useDashboardFilters();

  const [selectedPreset, setSelectedPreset] = useState<DatePreset>(state.preset);
  const [customStart, setCustomStart] = useState(
    state.startDate.slice(0, 10) || format(new Date(), "yyyy-MM-dd"),
  );
  const [customEnd, setCustomEnd] = useState(
    state.endDate.slice(0, 10) || format(new Date(), "yyyy-MM-dd"),
  );
